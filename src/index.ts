...
import { defaultDirectoryStore } from "./providers";

interface FileSystemAccessProps {
  filters?: FileFilterFn[];
  store?: IDirectoryStore<DirectoryData, DirectoryStoreOptions>;
...
export default function useFileSystemAccess(props: FileSystemAccessProps = {}) {
  ...
  const filtersRef = useRef<FileFilter[]>([]);
  ...
  const memoFiltersRef = useRef<FileFilter[]>([]);
  ...
  const initFileFilters = useCallback(async () => {
    const filterPromises: Promise<FileFilter>[] = [];
  ...
  const getSavedDirectories = async (
    providerOptions?: DirectoryStoreOptions
